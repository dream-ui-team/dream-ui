...
import { View, FlatList, ActivityIndicator,StyleSheet,Text,TouchableOpacity,Linking ,Picker } from "react-native";
import { NavigationScreenProp, NavigationState } from "react-navigation";
import { connect } from "react-redux";
import { Header,CommonButton } from "../../../components";
import styles from "./styles";
import { AvatarItem } from "../../../components";
import { logoutUserService,getAllLocationsService,getServiceCentresByLocationId ,getCostSheet} from "../../../redux/services/user";
...
class ServiceCenterDetails extends Component {
  ...
  getServiceCentres=()=>
  {
	  ...
	  getCostSheet(this.state.serviceCentres.partnerId).then(res =>{
  ...
  render() {
	...
	<View style={styles.container}>
	  ...
	  <View style={styles.homeContainerRow}>
		<Text style={styles.serviceDetailsText}>{this.state.serviceCentres.name}</Text>
		</View>
		<View style={styles.homeContainerRow}>
			 <Text style={styles.serviceDetailsText}>{this.state.serviceCentres.address}</Text>
			 </View>
			 <View style={styles.homeContainerRow}>
			  <Text style={styles.serviceDetailsText}>{this.state.serviceCentres.mobileNumber}</Text>
			  </View>
			  <View style={styles.homeContainerRow}>
			   <Text style={styles.serviceDetailsText}>{this.state.serviceCentres.alternateMobileNumber}</Text>
			   </View>
			   <View style={styles.homeContainerRow}>
			   <Text style={styles.serviceDetailsText}>{this.state.serviceCentres.emailAddress}</Text>
			   </View>
				</View>
				<Text></Text>
				<View style={styles.homeContainer}>
				<View style={styles.homeContainerRow}>
				<Text style={{marginTop:6,marginBottom:4}}>{'Services offered by service center :'}</Text>
				</View>
				...
				<TouchableOpacity  style={styles.buttonStyle} onPress={()=>this.getServiceCentres()}>
				...
				<View style={styles.homeContainerRow}>
				{this.state.url?
					<Text style={{color: 'blue',marginTop:20,fontSize:15}}
